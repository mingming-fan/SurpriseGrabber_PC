...
//    if ((eType == PT_WIN2000) || (eType == PT_WINNT))
    {
		...
		sprintf (tmp, "\\\\?\\COM%d", m_comm_port);
		//LPCSTR lpa = "\\\\?\\COM%d"; 
		size_t aLen = strlen( tmp ) + 1; 
...
int CSerialComm::ReadData( char* lpszBlock, int nMaxLength )
{
    BOOL       fReadStat ;
    COMSTAT    ComStat ;
    DWORD      dwErrorFlags;
